...
import br.com.heimdall.core.dto.PlanDTO;
import br.com.heimdall.core.dto.page.PlanPage;
...
public class PlanService {
     ...
     public Plan find(Long id) {
          ...
          HeimdallException.checkThrow(plan == null, ExceptionMessage.GLOBAL_RESOURCE_NOT_FOUND);
     ...
     public Plan save(PlanDTO planDTO) {
          ...
          if (plan.isDefaultPlan()) {
               ...
               HeimdallException.checkThrow(plans.stream().anyMatch(Plan::isDefaultPlan), ExceptionMessage.DEFAULT_PLAN_ALREADY_EXIST_TO_THIS_API);
          }
     ...
     public Plan update(Long id, PlanDTO planDTO) {
          ...
          if (plan.isDefaultPlan()) {
               List<Plan> plans = apiService.plansByApi(plan.getApi().getId());
               HeimdallException.checkThrow(plans.stream().anyMatch(p -> !p.getId().equals(planId) && p.isDefaultPlan()), ExceptionMessage.DEFAULT_PLAN_ALREADY_EXIST_TO_THIS_API);
          }
     ...
     public void delete(Long id) {
          ...
          HeimdallException.checkThrow(totalAppsAttached > 0, ExceptionMessage.PLAN_ATTACHED_TO_APPS);
