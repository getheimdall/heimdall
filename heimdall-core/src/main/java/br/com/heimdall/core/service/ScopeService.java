...
public class ScopeService {
    ...
    public Scope find(final Long apiId, final Long scopeId) {
        ...
        HeimdallException.checkThrow(scope == null, ExceptionMessage.GLOBAL_RESOURCE_NOT_FOUND);
    ...
    public Scope save(final Long apiId, Scope scope) {
        ...
        HeimdallException.checkThrow(scopeData != null, ExceptionMessage.SCOPE_INVALID_NAME);

        HeimdallException.checkThrow(scope.getOperations() == null || scope.getOperations().isEmpty(),
                ExceptionMessage.SCOPE_NO_OPERATION_FOUND);
        ...
        scope.getOperations().forEach(op -> {
            ...
            HeimdallException.checkThrow(
                    operation == null,
                    ExceptionMessage.SCOPE_INVALID_OPERATION, op.getId().toString());

            HeimdallException.checkThrow(
                    !operation.getResource().getApi().getId().equals(apiId),
                    ExceptionMessage.SCOPE_OPERATION_NOT_IN_API, operation.getId().toString(), apiId.toString());
        });
