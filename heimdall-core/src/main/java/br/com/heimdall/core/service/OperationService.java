...
public class OperationService {
     ...
     public Operation find(Long apiId, Long resourceId, Long operationId) {
          ...
          HeimdallException.checkThrow(operation == null, ExceptionMessage.GLOBAL_RESOURCE_NOT_FOUND);
     ...
     private Example<Operation> prepareExample(Long apiId, Long resourceId, OperationDTO operationDTO) {
         ...
         HeimdallException.checkThrow(resource == null, ExceptionMessage.GLOBAL_RESOURCE_NOT_FOUND);
     ...
     public Operation save(Long apiId, Long resourceId, Operation operation) {
          ...
          HeimdallException.checkThrow(resData != null &&
                  Objects.equals(resData.getResource().getId(), resource.getId()), ExceptionMessage.ONLY_ONE_OPERATION_PER_RESOURCE);

          boolean patternExists = operationJDBCRepository.patternExists(resource.getApi().getBasePath() + "/" + operation.getPath(), apiId);
          HeimdallException.checkThrow(patternExists, ExceptionMessage.OPERATION_ROUTE_ALREADY_EXISTS);

          operation.setResource(resource);
          operation.setPath(StringUtils.removeMultipleSlashes(operation.getPath()));

          HeimdallException.checkThrow(validateSingleWildCardOperationPath(operation), ExceptionMessage.OPERATION_CANT_HAVE_SINGLE_WILDCARD);
          HeimdallException.checkThrow(validateDoubleWildCardOperationPath(operation), ExceptionMessage.OPERATION_CANT_HAVE_DOUBLE_WILDCARD_NOT_AT_THE_END);
     ...
     public Operation update(Long apiId, Long resourceId, Long operationId, OperationDTO operationDTO) {
          ...
          HeimdallException.checkThrow(resData != null &&
                  ...
                  !resData.getId().equals(operation.getId()), ExceptionMessage.ONLY_ONE_OPERATION_PER_RESOURCE);
