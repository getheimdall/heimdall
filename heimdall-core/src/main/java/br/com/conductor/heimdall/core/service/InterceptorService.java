        ...
        switch (type) {
            ...
            case OAUTH:
                try {
                    ...
                    if (Objeto.isBlank(oAuthDTO.getTypeOAuth())) {
                        ExceptionMessage.TYPE_OAUTH_NOT_FOUND.raise(type.name(), ExceptionMessage.TYPE_OAUTH_NOT_FOUND.getMessage());
                    }

                    if (oAuthDTO.getTypeOAuth().equals(TypeOAuth.AUTHORIZE.getTypeOAuth())) {
                        if (Objeto.isBlank(oAuthDTO.getProviderId()) || oAuthDTO.getProviderId() == 0) {
                            ExceptionMessage.PROVIDER_NOT_FOUND.raise(type.name(), ExceptionMessage.PROVIDER_NOT_FOUND.getMessage());
                        }
                    }

                    if (oAuthDTO.getTypeOAuth().equals(TypeOAuth.GENERATE.getTypeOAuth()) || oAuthDTO.getTypeOAuth().equals(TypeOAuth.VALIDATE.getTypeOAuth())) {
                        if (Objeto.isBlank(oAuthDTO.getPrivateKey())) {
                            ExceptionMessage.PRIVATE_KEY_NOT_FOUND.raise(type.name(), ExceptionMessage.PRIVATE_KEY_NOT_FOUND.getMessage());
                        }
                    }
